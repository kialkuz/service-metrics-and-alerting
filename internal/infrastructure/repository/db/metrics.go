...
func NewDBStorage(db *dbWrapper.DB) *MemStorage {
...
// 			tx.Exec(ctx, query, *metric.Delta, metric.ID)
// 		case model.Gauge:
// 			query := "UPDATE metrics SET value = $1 WHERE id = $2"
// 			tx.Exec(ctx, query, *metric.Value, metric.ID)
// 		}
...
// 	return dbWrapper.Query(ctx, r.db, func(rows pgx.Rows) ([]model.Metrics, error) {
